...
// Write code to detect collision between player arrow and opponent
  for ( var i=0; i<playerArrows.length; i++){
    var PbaseCollision = Matter.SAT.collides(playerArrows[i].body, computerBase.body);

    var ParcherCollision = Matter.SAT.collides(playerArrows[i].body, computerArcher.body);

    var PcomputerCollision = Matter.SAT.collides(playerArrows[i].body, computer.body);

    if(PbaseCollision.collided|| ParcherCollision.collided || PcomputerCollision.collided){
...
function handleComputerArrowCollision() {
  ...
  for ( var i=0; i<playerArrows.length; i++){
    var CbaseCollision = Matter.SAT.collides(computerArrows[i].body, playerBase.body);

    var CarcherCollision = Matter.SAT.collides(computerArrows[i].body, playerArcher.body);

    var CplayerCollision = Matter.SAT.collides(computerArrows[i].body, player.body);

    if(CbaseCollision.collided|| CarcherCollision.collided || CplayerCollision.collided){
